...
managersData.forEach(function(manager) {
  ...
  requestsData.forEach(function(request, index) {
    requestsTable += "<tr data-entry-id='r" + index + "'><td>" + request.restaurantName + "</td><td>" + request.managerName + "</td><td>" + request.description + "</td><td>" + request.documents + "</td><td><button class='accept-btn'>Accept</button><button class='deny-btn' data-prid='" + index + "'>Deny</button></td></tr>";
...
document.querySelectorAll(".delete-btn").forEach(function(btn) {
    btn.addEventListener("click", function() {
        ...
        // Determine if it's a user or manager entry and construct the URL accordingly
        var url;
        if (entryId.startsWith("c")) {
            ...
            // Handle error if entryId is neither cid nor mid
            console.error("Invalid entry ID format:", entryId);
            return; // Exit function to prevent further execution
        }

        // Navigate to the delete_entry.php with appropriate parameters
...
document.querySelectorAll(".deny-btn").forEach(function(btn) {
        btn.addEventListener("click", function() {
            var row = this.closest("tr[data-entry-id]"); // Get the closest row with data-entry-id attribute
            var entryId = row.dataset.entryId; // Get the entry ID from data-entry-id attribute

            // Construct the URL with the entry ID
            var url = "deny_request.php?prid=" + encodeURIComponent(entryId);

            // Redirect to deny_request.php with parameters
            window.location.href = url;
        });
    });
